...
const static std::u8string langscorePluginDescription = u8"翻訳アプリLangscoreのRPGツクールMV/MZ用プラグインです。";
const static std::u8string langscoreObserverPluginDescription = u8"翻訳アプリLangscoreの補助プラグインです。必ず全てのプラグインの先頭に配置して下さい。";
...
void divisi_mvmz::updatePluginInfo()
{
    ...
    std::vector<nlohmann::ordered_json> pluginList;
    bool findLangscorePluginInfo = false;
    bool findLangscoreObserverPluginInfo = false;
    config config;
    const bool enableLangPatch = config.enableLanguagePatch();
    const bool enableTransDefLang = config.enableTranslationDefLang();

    for(auto& plugin : jsonObject)
    {
        //Langscoreが導入済みならパラメータを更新
        auto pluginName = plugin["name"];
        if(pluginName == "Langscore") {

            findLangscorePluginInfo = true;
            auto& params = plugin["parameters"];
    ...
    {
        //Langscoreのオブザーバープラグインを追加
        //他のプラグインがLangscoreのオブザーバーに追加できるようにするために、
        //必ず先頭に埋め込む。
        nlohmann::ordered_json newPlugin = {
            {"name", "Langscore_ObserverBridge"},
            {"status", true},
            {"description", cnvStr<std::string>(langscoreObserverPluginDescription)},
            {"parameters", {}}
        };
...
[
    ...
    for(size_t i = 0; i < pluginList.size(); ++i) {
        formattedJson << pluginList[i].dump();
...
utility::u8stringlist divisi_mvmz::formatSystemVariable(std::filesystem::path path)
{
    ...
    while(std::getline(inScriptFile, _linTmp))
    {
        ...
        if(findStr(_line, u8"%{DESCRIPTION}%")) {
            auto desc = langscorePluginDescription;
            desc = utility::replace(desc, u8"\r\n"s, u8"\r\n * "s);
            _line = langscorePluginDescription;
        }
        ...
        else if(findStr(_line, u8"%{ALLOWED_LANGUAGE}%"))
        {
            ...
            _line = utility::replace(_line, u8"%{ALLOWED_LANGUAGE}%"s, std::move(langs));
        }
